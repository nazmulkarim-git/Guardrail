  ...
  try {
    ...
    const rows = await getSql()`
      update waitlist_leads set
        ...
        source = coalesce(${normalizeString(body.source)}, source),
        plan_interest = coalesce(${normalizeString(body.planInterest) || normalizeString(body.plan_interest)}, plan_interest),
        framework_interest = coalesce(${normalizeString(body.frameworkInterest) || normalizeString(body.framework_interest)}, framework_interest),
        external_actions = coalesce(${normalizeString(body.externalActions) || normalizeString(body.external_actions)}, external_actions),
        founder_call_interest = coalesce(${normalizeString(body.founderCallInterest) || normalizeString(body.founder_call_interest)}, founder_call_interest),
